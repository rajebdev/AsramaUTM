		...
		<div class="form-row">
			<div class="col-md-6">
				<div class="position-relative form-group">
					...
					<select name="jenjangpendidikan" id="jenjangpendidikan" class="form-control">
						<?php foreach ($jenjangpendidikan as $key => $value) { ?>
							<option value="<?= $value->id_pendidikan ?>" <?= $value->id_pendidikan == $table['id_pendidikan'] ? 'selected' : '' ?>>
				...
				<div class="position-relative form-group">
					...
					<input name="nama_sekolah" id="nama_sekolah" type="text" class="form-control" value="<?= $table['nama_sekolah'] ?>" autofocus>
					<?= form_error('nama_sekolah', '<small class="text-danger pl-3">', '</small>') ?>
				</div>
				<div class="position-relative form-group">
					<label for="tahun_lulus" class="">Tahun Lulus</label>
					<input name="tahun_lulus" id="tahun_lulus" type="text" class="form-control" value="<?= $table['tahun_lulus'] ?>" autofocus>
